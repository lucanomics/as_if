...
export function PhraseCard(props: PhraseCardProps) {
  if (props.type === 'safe') {
    ...
    return (
      <article className="space-y-3 rounded-lg border border-[--color-line] bg-[--color-card] p-4">
        ...
        <p className="text-sm"><span className="font-medium">Use case:</span> {item.useCase}</p>
        <p className="text-sm"><span className="font-medium">Why it works:</span> {item.whyItWorks}</p>
        {item.caution ? <p className="text-sm text-amber-300"><span className="font-medium">Caution:</span> {item.caution}</p> : null}
  ...
  return (
    <article className="space-y-3 rounded-lg border border-[--color-line] bg-[--color-card] p-4">
      ...
      <p className="text-base font-semibold">“{item.riskyPhrase}”</p>
      <div className="flex flex-wrap gap-2">
        <span className={badgeClass}>{phraseCategoryLabels[item.category]}</span>
      ...
      <p className="text-sm"><span className="font-medium">Why risky:</span> {item.whyRisky}</p>
      <p className="text-sm"><span className="font-medium">Safer alternative:</span> “{item.saferAlternative}”</p>
      <p className="text-sm"><span className="font-medium">Explanation:</span> {item.explanation}</p>
      <div className="flex flex-wrap gap-2">{item.tags.map((tag) => <span key={tag} className={badgeClass}>#{tag}</span>)}</div>
    </article>
  )
}
